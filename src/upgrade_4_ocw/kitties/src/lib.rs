...
pub mod pallet {
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn extrinsic(origin: OriginFor<T>, number: u64) -> DispatchResult {
			...
			log::info!("offchain saved key: {:?}", key);
			log::info!("offchain saved value: {:?}", data.encode());
	...
	impl<T: Config> Pallet<T> {
		fn get_next_id() -> Result<KittyId, DispatchError> {
			NextKittyId::<T>::try_mutate(|next_id| -> Result<KittyId, DispatchError> {
				...
				*next_id = next_id.checked_add(1).ok_or::<DispatchError>(Error::<T>::InvalidKittyId.into())?; // 超出u32值的范围时报错
				Ok(current_id)
			})
		}
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		...
		fn offchain_worker(block_number: T::BlockNumber) {
			...
			log::info!("offchain restore key: {:?}", key);
